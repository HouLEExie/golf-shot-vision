...
class BallDetector:
    ...
    def detect_candidates(
        ...
        if previous_frame is not None:
            ...
            if cv2.countNonZero(candidate_mask) < 3:
        ...
        max_area = max(40.0, min(900.0, frame_area * 0.00045))

        for contour in contours:
            area = float(cv2.contourArea(contour))
            if area < 1.0 or area > max_area:
                continue
            ...
            size_score = 1.0 - min(1.0, abs(radius - 4.0) / 16.0)
            circularity = 0.0
            ...
            if previous_frame is not None:
                ...
                motion_score = min(1.0, abs(float(gray[y_i, x_i]) - previous_value) / 60.0)

            score = (0.42 * brightness_score) + (0.24 * size_score) + (0.18 * circularity) + (0.16 * motion_score)
            detections.append(
                Detection(
