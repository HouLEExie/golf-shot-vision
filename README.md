...
如果自动识别不稳定，推荐使用“半自动识别，推荐”模式，并填写：

- 起始帧 `start_frame`
- 球起始位置 `start_x`、`start_y`
- 搜索区域 ROI：`roi_x1`、`roi_y1`、`roi_x2`、`roi_y2`
- 飞行方向：左到右、右到左或向上
- 最大单帧位移 `max_step_px`
- 最小单帧位移 `min_step_px`
- 轨迹平滑强度 `smoothness`

如果 ROI 未设置，系统会使用全画面识别，但更容易误识别球杆反光、地面白点或背景亮点。

30fps 视频可以粗略分析，但球飞行过程中帧间位移很大，不建议用于准确距离估算。若要提高识别稳定性，建议使用 iPhone 慢动作 120fps 或 240fps 重新拍摄。
